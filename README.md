...
1. **Install the bot** — the CLI opens a Discord authorization page in your browser. Pick your server and click **Authorize**. This installs the shared Kimaki bot into your server with one click — no Discord Developer Portal setup needed.
2. **Pick your projects** — the CLI asks which folders on your machine should become Discord channels. Each channel is one project.
3. **Done** — the bot starts and is ready. Send a message in a project channel to start a coding session.

> **Keep the terminal running.** The CLI process is the bridge between Discord and your machine — it runs the bot and the agent sessions. Closing it stops the bot.
...
Kimaki gives you access to **every model omo supports**: Anthropic, OpenAI, Google, and more. The best part: you can use your existing **Claude Pro/Max** and **ChatGPT/Codex** subscriptions instead of paying per token.
...
Kimaki adds a layer of orchestration features on top of the agent runtime. The ones worth knowing first:
...
- **[Agent commands](https://kimaki.dev/docs/features/opencode-commands)** — your agent commands, skills, and MCP prompts become Discord slash commands.
...
If sessions stop responding, fail to start, or the bot behaves unexpectedly, run `/restart-opencode-server` in any channel. This restarts the backend agent runtime while keeping the bot connected to Discord. It fixes most transient issues.
